...
def positional(max_positional_args):
    ...
    if isinstance(max_positional_args, sixm.integer_types):
...
def scopes_to_string(scopes):
    ...
    """
    if isinstance(scopes, sixm.string_types):
        return scopes
    else:
        return ' '.join(scopes)
...
def string_to_scopes(scopes):
    ...
    if not scopes:
        return []
    elif isinstance(scopes, sixm.string_types):
        return scopes.split(' ')
    else:
        return scopes
...
def parse_unique_urlencoded(content):
    ...
    params = {}
    for key, value in sixm.iteritems(urlencoded_params):
        if len(value) != 1:
...
def _to_bytes(value, encoding='ascii'):
    ...
    Unfortunately, ``sixm.b`` is insufficient for this task since in
    ...
    result = (value.encode(encoding)
              if isinstance(value, sixm.text_type) else value)
    if isinstance(result, sixm.binary_type):
        return result
    else:
...
def _from_bytes(value):
    ...
    result = (value.decode('utf-8')
              if isinstance(value, sixm.binary_type) else value)
    if isinstance(result, sixm.text_type):
        return result
    else:
        raise ValueError(
